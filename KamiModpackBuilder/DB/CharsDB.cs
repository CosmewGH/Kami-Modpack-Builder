...
namespace KamiModpackBuilder.DB
{
    public class CharsDB
    {
        ...
        #region Main Methods
        internal static void InitializeCharsDB(int gameVersion, bool isSwitch)
        {
            if (isSwitch)
                InitializeCharsSwitchDB(gameVersion);
            else
        ...
        private static void InitializeCharsSwitchDB(int gameVersion)
        {
            throw new Exception(string.Format("CharsDB: Version {0} for Ultimate not supported.", gameVersion));
        }
        #endregion
