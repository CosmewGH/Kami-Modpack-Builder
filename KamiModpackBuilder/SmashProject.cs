    ...
    public class SmashProjectManager
    {
        ...
        internal void RebuildRFAndPatchlist(bool packing)
        {
            ...
            foreach (CharacterSlotMod mod in _CurrentProject.ActiveCharacterSlotMods)
            {
                ...
                string explorerFighterFolder = explorerFolder + "data" + Path.DirectorySeparatorChar + "fighter" + Path.DirectorySeparatorChar + (string.IsNullOrEmpty(currentFighter.nameFighterFolder) ? fighterName.ToLower() : currentFighter.nameFighterFolder) + Path.DirectorySeparatorChar;
                ...
                if (xml.Sound)
                {
                    if (currentFighter.soundPackSlots == Fighter.SoundPackSlots.All)
                    {
                        ...
                        if (fileSearch.Count > 0)
                        {
                            ...
                            nus3Banks.Add(new AutoMTBFix.Nus3BankFile { fighterName = (string.IsNullOrEmpty(currentFighter.nameSoundPack) ? currentFighter.name : currentFighter.nameSoundPack), fileName = fileSearch[0], cxxNum = (ushort)mod.SlotID });
                        }
                    }
                    else
                    {
                        if (_CurrentProject.GetAudioSlotForFighter(currentFighter.id, false, 0) == mod.SlotID)
                        {
                            ...
                            if (fileSearch.Count > 0)
                            {
                                ...
                                explorerFilename = explorerFighterFolder + "sound" + Path.DirectorySeparatorChar + explorerFilename;
                                AddFileToResColFileLists(explorerFilename, fileSearch[0], baseFolders, filesLists);
                                nus3Banks.Add(new AutoMTBFix.Nus3BankFile { fighterName = (string.IsNullOrEmpty(currentFighter.nameSoundPack) ? currentFighter.name : currentFighter.nameSoundPack), fileName = fileSearch[0], cxxNum = 0 });
                            }
                        }
                        else if (currentFighter.soundPackSlots == Fighter.SoundPackSlots.Two && _CurrentProject.GetAudioSlotForFighter(currentFighter.id, false, 1) - 1 == mod.SlotID)
                        {
                            fileSearch.AddRange(Directory.GetFiles(modFolder + "sound", "*snd_se_*", SearchOption.AllDirectories));
                            if (fileSearch.Count > 0)
                            {
                                ...
                                explorerFilename = explorerFighterFolder + "sound" + Path.DirectorySeparatorChar + explorerFilename;
                                AddFileToResColFileLists(explorerFilename, fileSearch[0], baseFolders, filesLists);
                                nus3Banks.Add(new AutoMTBFix.Nus3BankFile { fighterName = (string.IsNullOrEmpty(currentFighter.nameSoundPack) ? currentFighter.name : currentFighter.nameSoundPack), fileName = fileSearch[0], cxxNum = 1 });
                            }
                        }
                    }
                    fileSearch.Clear();
                ...
                if (xml.Voice)
                {
                    ...
                    else
                    {
                        ...
                        else if (currentFighter.voicePackSlots == Fighter.VoicePackSlots.Two && _CurrentProject.GetAudioSlotForFighter(currentFighter.id, true, 1) - 1 == mod.SlotID)
                        {
                            fileSearch.AddRange(Directory.GetFiles(modFolder + "sound", "*snd_vc_*", SearchOption.AllDirectories));
                            if (fileSearch.Count > 0)
                            {
            ...
            foreach (StageSlotMod mod in _CurrentProject.ActiveStageMods)
            {
                ...
                string explorerStageUIFolderDLC = explorerFolder + "data" + Path.DirectorySeparatorChar + "ui" + Path.DirectorySeparatorChar + "replace" + Path.DirectorySeparatorChar + "append" + Path.DirectorySeparatorChar + "stage" + Path.DirectorySeparatorChar;
                ...
                if (Directory.Exists(modFolder + "ui"))
                {
                    ...
                    foreach (string f in fileSearch)
                    {
                        ...
                        AddFileToResColFileLists(explorerFilename, f, baseFolders, filesLists);
                        if (currentStage.IsDLC)
                        {
                            explorerFilename = explorerFilename.Replace(explorerStageUIFolder, explorerStageUIFolderDLC);
                            AddFileToResColFileLists(explorerFilename, f, baseFolders, filesLists);
                        }
                    }
                    fileSearch.Clear();
                    ...
                    fileSearch.Clear();
                    fileSearch.AddRange(Directory.GetFiles(modFolder + "ui", "*stage_13*", SearchOption.AllDirectories));
                    foreach (string f in fileSearch)
                    {
                        string explorerFilename = f.Replace(modFolder + "ui" + Path.DirectorySeparatorChar, string.Empty);
                        explorerFilename = explorerFilename.Replace("XX", stageName);
                        explorerFilename = explorerStageUIFolder + "stage_13" + Path.DirectorySeparatorChar + explorerFilename;
                        AddFileToResColFileLists(explorerFilename, f, baseFolders, filesLists);
                        if (currentStage.IsDLC)
                        {
                            explorerFilename = explorerFilename.Replace(explorerStageUIFolder, explorerStageUIFolderDLC);
